...
def none():
 ...
 {B2} i 4 m _ a a d o d 
{B2} i4m adod
{B2} i4m adod  
{H2} i4m adod
 {H2}i4m adod
         Fucked By i4m_aadod {P2}Coder""",width=80,style=f"{color_panel}"))
def info():
	prints(f"""----------------------------------
 OWNER 👉 i4m_aadod
 TlGRAM 👉i4m_aadod_iraq
----------------------------------""")
print('Fucked By i4m_adod')
def banner():
	clear()
	prints(nel(f"""\t {color_text} 
-----------------------------------------------
OWNER 👉 i4m_adod
-----------------------------------------------
         Made By {M2}ADOD {P2}Coder""",width=80,style=f"{color_panel}"))
def login():
	try:
...
def login_lagi334():
	try:
		...
		print('\n╰─ Cookies TAWA DAHL BO √✓ ')
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print(f'\033[0m╰─Cookie Sotawa ××')
		exit()
...
def menu(my_name,my_id):
	...
	prints(nel(f"""{P2}[{color_text} 1 {P2}]. Crack Public
{P2}[{color_text} 2 {P2}]. Crack Public/MULTI
{P2}[{color_text} 3 {P2}]. Exit\n""",width=80,padding=(0,7),style=f"{color_panel}"))
	...
	if Meledak in ['1']:
		...
		print('╰─ Successfully Dargo+Srawa Cookies✓√ ')
		exit()
	else:
...
def dump_publik():
	try:
		...
		exit()
	prints(nel(f'"\033[32mMe" in Username/id:                    {P2}',width=90,padding=(0,9),style=f"{color_panel}")) 
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('╰─ GAND IDS DADAN BO CRACK (15max) ? : '))
	except ValueError:
		print('╰─ wrong input ')
		exit()
	if jum<1 or jum>100:
...
{B2}╰─────────────────────╯""")
	...
	print('')
	prints(nel(f'''\t{P2}[{H2}1{P2}] Methode Mobile\"''',width=80,style=f"{color_panel}")) 
	hc = input('[•] Pilih  : ')
	if hc in ['1','01']:
	...
	su() 
def su():
	prints(nel(f"\t{P2}[{H2}1{P2}].   321 + 123 + 12345 [ {K2}SLOW (BEST✓)",width=80,style=f"{color_panel}")) 
	ch = input('[•] Pilih  : ')
	if ch in ['1','01']:
		mie()
	elif ch in ['2','02']:
...
def crackfree(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	prog.update(des,description=f'\r[deep_white] i4m-> AADOD{(loop)}/{len(id)}[/] [green]OK[/]:[green]{(ok)} [/]=[yellow] CP[/]:[yellow]{(cp)}')
	prog.advance(des)
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
...
def getkey():
    ...
    try:
        ...
        none();time.sleep(1)
        print("")
        print("\033[0m╰─ Author: ITZ_AADOD ")
        print("\033[0m╰─ Telegram: i4m_adod ");time.sleep(2)
        print ("")
...
###----------[ AUTHOR ]---------- ###
Author = 'DADOD-DECODER'
Version = 5.1
Facebook = 'Facebook.com/ADODCRACKER'
Instagram = 'Instagram.com/itz_aadod'
